...
| Harness contract | `lib/agent-harness.ts` | providers, capabilities, workflows, events, generated artifacts |
| API surface | `app/api/harness/route.ts`, `app/api/generate/route.ts` | workflow discovery and deterministic mock generation |
| Studio UI | `app/app`, `components/studio/*` | OpenDesign-inspired workspace and artifact canvas |
...
2. `/api/generate` accepts a workflow id and product idea, then returns mock events and typed artifacts.
3. `/app` renders the current FinSight demo workspace using the existing visual design baseline.
...
1. Convert the deterministic FinSight mock into a typed Product Pack object shared by API and UI.
2. Add PRD-to-Prototype linked view data from the harness response instead of static component data.
3. Add Codex / Claude Code provider detection behind the current adapter contract.
4. Add export actions for Markdown, HTML, PDF, and PPTX after the artifact model is stable.
