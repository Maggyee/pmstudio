...
export function buildBlankProductPack(): ProductPack {
  const generatedAt = "2026-06-20T00:00:00+08:00";
  const artifactIndex = cloneArtifactIndex().map((artifact) => ({
    ...artifact,
  ...
  return {
    ...
    prototype: {
      ...
      screens: [
        {
          ...
          goal: "等待输入产品想法后生成原型。",
          primaryAction: "生成方案",
          components: ["输入产品想法", "运行工作流", "查看生成结果"],
        },
      ],
      prdLinks: [],
      openDesignPrompt: "",
      liveArtifact: {
        schemaVersion: "open-design-live-artifact.v1",
        id: "blank-prototype-preview",
        title: "Blank Prototype Preview",
        preview: {
          type: "html",
          entry: "index.html",
        },
